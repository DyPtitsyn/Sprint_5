...
class TestBurgersGoToLk:
    def test_go_to_lk(self, driver):
        ...
        lk_button = driver.find_element(By.XPATH, BurgersLocators.LK_BUTTON)
        lk_button.click()
        WebDriverWait(driver, 5).until(expected_conditions.visibility_of_element_located((By.XPATH, BurgersLocators.SAVE_BUTTON)))
        lk_name = driver.find_element(By.XPATH, BurgersLocators.LK_NAME)
        assert lk_name.is_displayed() and lk_name.get_attribute('value') == 'Dima'
